...
template<typename NetworkT, typename DistType, typename NbHopsType> std::pair<
		DistType, NbHopsType> ESPDistCalculator<NetworkT, DistType, NbHopsType>::getDist(
	...
	case NoCache: {
		return dijkstra.getDist(i, lengthMapId,
				...
				NetDistCalculator<NetworkT, DistType, NbHopsType>::discNbHops)->at(
				j);
	}
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::pair<
		DistType, NbHopsType> ESPDistCalculator<NetworkT, DistType, NbHopsType>::getIndDist(
		NodeIdType const & i, NodeIdType const & j) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeDistMapSP ESPDistCalculator<
		NetworkT, DistType, NbHopsType>::getDist(NodeIdType const & i) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeDistMapSP ESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::getDist(NodeIdType const & i) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::pair<
		DistType, NbHopsType> ESPLDistCalculator<NetworkT, DistType, NbHopsType>::getDist(
		NodeIdType const & i, NodeIdType const & j) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeSDistMapSP ESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::getSDistMap(NodeIdType const & i) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeSDistMapSP ESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::getFinDistMapNoNeighb(
		NodeIdType const & srcNode) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::pair<
		DistType, NbHopsType> ASPDistCalculator<NetworkT, DistType, NbHopsType>::getDist(
		NodeIdType const & i, NodeIdType const & j) {
	auto dst = NetDistCalculator<NetworkT, DistType, NbHopsType>::discDist;
	auto nbHops = NetDistCalculator<NetworkT, DistType, NbHopsType>::discNbHops;
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ASPDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeDistMapSP ASPDistCalculator<
		NetworkT, DistType, NbHopsType>::getDist(NodeIdType const & i) {
	auto net = dijkstra.getNet();
	auto dist =
			net->template createNodeMapSP<std::pair<DistType, NbHopsType>>();
	for (NodeIdType j = 0; j < net->getNbNodes(); j++) {
		(*dist)[j] = getDist(i, j);
	}
	return dist;
}

template<typename NetworkT, typename DsimType, typename NbHopsType> std::pair<
		DsimType, NbHopsType> ESPDsimCalculator<NetworkT, DsimType, NbHopsType>::getDist(
		NodeIdType const & i, NodeIdType const & j) {
	switch (cacheLevel) {
	case NoCache: {
		return dijkstra.getDsim(i, lengthMapId,
				NetDistCalculator<NetworkT, DsimType, NbHopsType>::discDist,
				NetDistCalculator<NetworkT, DsimType, NbHopsType>::discNbHops)->at(
				j);
	}
...
#endif
		{
			...
			if (fit != netDsimCache.end()) {
				...
				if (fit != netDsimCache.end()) {
					...
					auto dsim =
							dijkstra.getDsim(i, lengthMapId,
									NetDistCalculator<NetworkT, DsimType,
											NbHopsType>::discDist,
									NetDistCalculator<NetworkT, DsimType,
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::pair<
		DistType, NbHopsType> ESPDsimCalculator<NetworkT, DistType, NbHopsType>::getIndDist(
		NodeIdType const & i, NodeIdType const & j) {
...
// TODO add omp critical
template<typename NetworkT, typename DsimType, typename NbHopsType> typename ESPDsimCalculator<
		NetworkT, DsimType, NbHopsType>::NodeDsimMapSP ESPDsimCalculator<
		NetworkT, DsimType, NbHopsType>::getDist(NodeIdType const & i) {
#ifdef WITH_OPENMP
...
template<typename NetworkT, typename DsimType, typename NbHopsType> std::pair<
		DsimType, NbHopsType> ASPDsimCalculator<NetworkT, DsimType, NbHopsType>::getDist(
		NodeIdType const & i, NodeIdType const & j) {
	auto dst = NetDistCalculator<NetworkT, DsimType, NbHopsType>::discDist;
	auto nbHops = NetDistCalculator<NetworkT, DsimType, NbHopsType>::discNbHops;
...
template<typename NetworkT, typename DsimType, typename NbHopsType> typename ASPDsimCalculator<
		NetworkT, DsimType, NbHopsType>::NodeDsimMapSP ASPDsimCalculator<
		NetworkT, DsimType, NbHopsType>::getDist(NodeIdType const & i) {
	auto net = dijkstra.getNet();
	auto dist =
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::tuple<
		DistType, DistType, NbHopsType> ESPSimlCalculator<NetworkT, DistType,
		NbHopsType>::getSiml(NodeIdType const & i, NodeIdType const & j,
		NodeDistMapSP const simi, NodeDistMapSP const simj) const {

	double simij = 1;
	auto dhij = simi->at(j);
	for (auto it = net->neighborsBegin(j); it != net->neighborsEnd(j); ++it) {
	...
	double simji = 1;
	auto dhji = simj->at(i);
	for (auto it = net->neighborsBegin(i); it != net->neighborsEnd(i); ++it) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::tuple<
		DistType, DistType, NbHopsType> ESPSimlCalculator<NetworkT, DistType,
		NbHopsType>::getDirSiml(NodeIdType const & i, NodeIdType const & j,
 ...
 */
template<typename NetworkT, typename DistType, typename NbHopsType> std::tuple<
		DistType, DistType, NbHopsType> ESPLSimlCalculator<NetworkT, DistType,
		NbHopsType>::getSiml(NodeIdType const & i, NodeIdType const & j,
	...
	if (dhij.second == discNbHops) {
		...
		for (auto it = net->neighborsBegin(j); it != net->neighborsEnd(j);
				++it) {
			auto res = simi->at(net->end(*it));
			auto h = res.second;
			if (h != discNbHops) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPLSimlCalculator<
		NetworkT, DistType, NbHopsType>::NodeSDistMapSP ESPLSimlCalculator<
		NetworkT, DistType, NbHopsType>::getSDistMap(NodeIdType const & i) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPLSimlCalculator<
		NetworkT, DistType, NbHopsType>::NodeSDistMapSP ESPLSimlCalculator<
		NetworkT, DistType, NbHopsType>::getNnzSimlMap(
		NodeIdType const & srcNode) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename ESPLSimlCalculator<
		NetworkT, DistType, NbHopsType>::NodeSDistMapSP ESPLSimlCalculator<
		NetworkT, DistType, NbHopsType>::getNnzSimlMapNoNeighb(
		NodeIdType const & srcNode) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::tuple<
		DistType, DistType, NbHopsType> ESPIndSimlCalculator<NetworkT, DistType,
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::tuple<
		DistType, DistType, NbHopsType> ESPIndSimlCalculator<NetworkT, DistType,
		NbHopsType>::getIndSiml(NodeIdType const & i, NodeIdType const & j) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::tuple<
		DistType, DistType, NbHopsType> ESPIndSimlCalculator<NetworkT, DistType,
		NbHopsType>::getDirIndSiml(NodeIdType const & i, NodeIdType const & j) {
...
////////////////////////////////////////////////////////////////////////////
template<typename NetworkT, typename DistType, typename NbHopsType> typename DESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeDistMapSP DESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::getDist(NodeIdType const & i) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> std::pair<
		DistType, NbHopsType> DESPLDistCalculator<NetworkT, DistType, NbHopsType>::getDist(
		NodeIdType const & i, NodeIdType const & j) {
...
template<typename NetworkT, typename DistType, typename NbHopsType> typename DESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::NodeSDistMapSP DESPLDistCalculator<
		NetworkT, DistType, NbHopsType>::getSDistMap(NodeIdType const & i) {
