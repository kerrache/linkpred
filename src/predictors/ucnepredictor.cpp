...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void UCNEPredictor<
...
#pragma omp parallel for if (parallel)
	...
	for (auto it = begin; it < end; ++it) {
		...
		*(scores + (it - begin)) = net->getNbCommonNeighbors(srcNode, endNode);
	}
}

template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> double UCNEPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t UCNEPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
		...
		for (auto nnit = net->neighborsBegin(j); nnit != net->neighborsEnd(j);
			...
			if (i < k) {
				...
				if (!net->isEdge(e) && mqs[ind].count(e) == 0) {
					auto sc = net->getNbCommonNeighbors(i, k);
					mqs[ind].push(e, sc);
				}
			}
		}
...
#define UCNEPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef UCNEPREDICTOR_CPP
