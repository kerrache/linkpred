...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void DPATPredictor<
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> double DPATPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
...
/*template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t DPATPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
				...
				for (auto nnit = net->neighborsBegin(j); nnit != net->neighborsEnd(j);
					...
					if (i < k) {
						...
						if (!net->isEdge(e) && mqs[ind].count(e) == 0) {
							auto sc = (double) net->getOutDeg(i) * net->getInDeg(k);
							mqs[ind].push(e, sc);
						}
					}
				}
