...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void UPOPPredictor<
...
#pragma omp parallel for if (parallel)
	...
	for (auto it = begin; it < end; ++it) {
		...
		*(scores + (it - begin)) = pop(ki, kj);
	}
}

template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> double UPOPPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
...
#define UPOPPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef UPOPPREDICTOR_CPP
