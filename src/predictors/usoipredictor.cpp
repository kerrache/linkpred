...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void USOIPredictor<
...
#pragma omp parallel for if (parallel)
	...
	for (auto it = begin; it < end; ++it) {
		...
		*(scores + (it - begin)) = 2.0
				* net->getNbCommonNeighbors(srcNode, endNode)
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t USOIPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
		...
		for (auto nnit = net->neighborsBegin(j); nnit != net->neighborsEnd(j);
			...
			if (i < k) {
				...
				if (!net->isEdge(e) && mqs[ind].count(e) == 0) {
					auto sc = 2.0 * net->getNbCommonNeighbors(i, k)
...
#define USOIPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef USOIPREDICTOR_CPP
