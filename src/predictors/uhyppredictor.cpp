...
template<typename NetworkT, typename EdgesRandomIteratorT,
	...
	for (auto it = net->edgesBegin(); it != net->edgesEnd(); ++it) {
		auto vertex1 = NetworkT::start(*it);
		auto vertex2 = NetworkT::end(*it);
...
#pragma omp parallel for if (parallel)
	...
	// Power law fitting
	gamma = Utilities::plFit(degs).first;
...
#pragma omp parallel for if (parallel)
	...
	for (auto it = begin; it < end; ++it) {
		auto coordsi = nodesCoord.at(NetworkT::start(*it));
		auto coordsj = nodesCoord.at(NetworkT::end(*it));
