 ...
 * Copyright (C) 2017  by JIDd Kerrache.
 *
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void DJIDPredictor<
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> double DJIDPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
	...
	double degI = net->getOutDeg(i);
	double degJ = net->getInDeg(j);
	return nbcn / ((double)(degI + degJ - nbcn) + std::numeric_limits<double>::epsilon());
}

template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t DJIDPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
...
#pragma omp parallel for if (parallel) schedule(dynamic) // We need a dynamic schedule because iteration vary greatly in duration
	...
	for (auto eit = start; eit < end; eit += step) {
		...
		auto j = net->end(*eit);
		for (auto nnit = net->inNeighborsBegin(i); nnit != net->inNeighborsEnd(i);
				++nnit) {
		...
		for (auto nnit = net->outNeighborsBegin(j); nnit != net->outNeighborsEnd(j);
				++nnit) {
			auto k = net->end(*nnit);
...
#define DJIDPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef DJIDPREDICTOR_CPP
