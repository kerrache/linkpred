...
CreateGroup(struct group *part, int label) {
	...
	(part->next)->nodeList = (struct node_lis *) calloc(1,
			sizeof(struct node_lis));
	(part->next)->next = NULL;
...
/*
 ...
 ---------------------------------------------------------------------
 */
int MoveNode(struct node_gra *node, struct group *old, struct group *newg) {
	if (RemoveNodeFromGroup(old, node) == 0)
		return 0;

	AddNodeToGroup(newg, node);
	return 1;
}
...
BuildNetFromGroup(struct group *group) {
	...
	struct node_gra *newg = NULL;
	struct node_lis *p = (group->nodeList);

	last = net = CreateHeaderGraph();

	while ((p = p->next) != NULL) {
		newg = CreateNodeGraph(last, p->nodeLabel);
		newg->state = p->ref->state;
		newg->coorX = p->ref->coorX;
		newg->coorY = p->ref->coorY;
		newg->coorZ = p->ref->coorZ;
		newg->ivar1 = p->ref->ivar1;
		newg->dvar1 = p->ref->dvar1;
		CopyAdjacencyList(p->ref, newg);
		last = newg;
	}
...
BuildNetFromGroupNeig(struct group *group) {
	...
	while ((p = p->next) != NULL) {
		/* Add the node */
		newg = CreateNodeGraph(last, p->ref->label);
		newg->state = p->ref->state;
		newg->coorX = p->ref->coorX;
		newg->coorY = p->ref->coorY;
		newg->coorZ = p->ref->coorZ;
		newg->inGroup = p->ref->inGroup;
		last = newg;
		CopyAdjacencyList(p->ref, newg);
		...
		while ((nei = nei->next) != NULL) {
			...
			if ((IsThereNode(nei->ref->label, net) == 0)
				...
				newg = CreateNodeGraph(last, nei->ref->label);
				newg->state = nei->ref->state;
				newg->coorX = nei->ref->coorX;
				newg->coorY = nei->ref->coorY;
				newg->coorZ = nei->ref->coorZ;
				newg->inGroup = nei->ref->inGroup;
				last = newg;
				CopyAdjacencyList(nei->ref, newg);
			}
		}
	}
...
void MapPartToNetSoft(struct group *part, struct node_gra *net) {
	...
	while ((g = g->next) != NULL) {
		...
		while ((nod = nod->next) != NULL) {
			...
			node = (*(struct node_tree **) tfind((void *) treeNode, &nodeDict,
...
void FPrintPartition(FILE *outf, struct group *partition, int list_sw) {
	...
	while ((g = g->next) != NULL) {
		if (g->size > 0) {
			...
			if (list_sw == 0) {
				fprintf(outf, "%d %d %d %d %d %lf %lf %lf ---", g->label + 1,
						g->size, g->totlinks, g->inlinks, g->outlinks,
...
double ModularityWeight(struct group *part) {
	...
	while ((g = g->next) != NULL)
		modul += (double) (2 * g->inlinksW) / (double) links2
				- (g->inlinksW + g->totlinksW) * (g->inlinksW + g->totlinksW)
						/ (links2 * links2);
 ...
 */
struct group *
SAGroupSplit(struct group *targ, double Ti, double Tf, double Ts,
	...
	if (cluster_sw == 1 && /*  cluster switch is on */
		...
		if (totallinks > 0) {
			...
			while (T > Tf) {
				...
				for (i = 0; i < nnod; i++) {
					...
					dE = 0.0;
					dE -=
							(double) (2 * glist[oldg]->inlinks)
									/ (double) totallinks
									- (double) (glist[oldg]->totlinks
											+ glist[oldg]->inlinks)
											* (double) (glist[oldg]->totlinks
													+ glist[oldg]->inlinks)
											/ ((double) totallinks
													* (double) totallinks);
					dE -=
							(double) (2 * glist[newg]->inlinks)
									/ (double) totallinks
									- (double) (glist[newg]->totlinks
											+ glist[newg]->inlinks)
											* (double) (glist[newg]->totlinks
													+ glist[newg]->inlinks)
											/ ((double) totallinks
													* (double) totallinks);
					dE +=
							(double) (2 * glist[oldg]->inlinks - 2 * inold)
									/ (double) totallinks
									- (double) (glist[oldg]->totlinks
											+ glist[oldg]->inlinks - nlink)
											* (double) (glist[oldg]->totlinks
													+ glist[oldg]->inlinks
													- nlink)
											/ ((double) totallinks
													* (double) totallinks);
					dE +=
							(double) (2 * glist[newg]->inlinks + 2 * innew)
									/ (double) totallinks
									- (double) (glist[newg]->totlinks
											+ glist[newg]->inlinks + nlink)
											* (double) (glist[newg]->totlinks
													+ glist[newg]->inlinks
													+ nlink)
											/ ((double) totallinks
...
SACommunityIdent(struct node_gra *net, double Ti, double Tf, double Ts,
	...
	while (T > Tf && count < limit) {
		...
		for (i = 0; i < cicle1; i++) {
			...
			dE = 0.0;
			dE -= (double) (2 * glist[oldg]->inlinks) / (double) totallinks
					- (double) (glist[oldg]->totlinks + glist[oldg]->inlinks)
							* (double) (glist[oldg]->totlinks
									+ glist[oldg]->inlinks)
							/ ((double) totallinks * (double) totallinks);
			dE -= (double) (2 * glist[newg]->inlinks) / (double) totallinks
		...
		if (collective_sw == 1) {
			for (i = 0; i < cicle2; i++) {
				...
				if (glist[g1]->size < nnod) { /*  Unless all nodes are together */
					...
					dE = 0.0;
					dE -=
							(double) (2 * glist[g1]->inlinks)
									/ (double) totallinks
									- ((double) (glist[g1]->totlinks
											+ glist[g1]->inlinks)
											* (double) (glist[g1]->totlinks
													+ glist[g1]->inlinks))
											/ ((double) totallinks
													* (double) totallinks);
					dE -=
							(double) (2 * glist[g2]->inlinks)
									/ (double) totallinks
									- ((double) (glist[g2]->totlinks
											+ glist[g2]->inlinks)
											* (double) (glist[g2]->totlinks
													+ glist[g2]->inlinks))
											/ ((double) totallinks
				...
				if (empty >= 0) { /*  if there are no empty groups, do nothing */
					...
					dE = 0.0;
					dE -=
							(double) (2 * glist[dice]->inlinks)
									/ (double) totallinks
									- ((double) (glist[dice]->totlinks
											+ glist[dice]->inlinks)
											* (double) (glist[dice]->totlinks
													+ glist[dice]->inlinks))
											/ ((double) totallinks
													* (double) totallinks);
					dE -=
							(double) (2 * glist[empty]->inlinks)
									/ (double) totallinks
									- ((double) (glist[empty]->totlinks
											+ glist[empty]->inlinks)
											* (double) (glist[empty]->totlinks
													+ glist[empty]->inlinks))
											/ ((double) totallinks
...
/*
 ...
 ---------------------------------------------------------------------
 */
double WithinModuleRelativeDegree(struct node_gra *node, struct group *part) {
	struct node_lis *p;
	double z;
