/*
 graph.c
 $LastChangedDate: 2011-07-11 12:05:30 +0200 (Mon, 11 Jul 2011) $
 $Revision: 248 $
 */
...
#include "linkpred/predictors/usbmpredictor/datastruct.h"
...
// ---------------------------------------------------------------------
struct node_gra *
CreateNodeGraph(struct node_gra *p, const char *label) {
...
void CopyAdjacencyList(struct node_gra *nori, struct node_gra *ndes) {
	...
	while ((pori = pori->next) != NULL) {
		AddAdjacencySoft(ndes, pori->nodeLabel, 1, 0, pori->weight,
				pori->status);
	}
}
...
MakeLabelDict(struct node_gra *net) {
	...
	while ((p = p->next) != NULL) {
		...
		treeNode = *(struct node_tree **) tsearch((void *) treeNode, &nodeDict,
...
// ---------------------------------------------------------------------
struct node_gra *
FBuildNetwork(FILE *inFile, int weight_sw, int auto_link_sw, int add_weight_sw,
		int symmetric_sw) {
	...
	while (!feof(inFile)) {
		...
		ntree1 = *(struct node_tree **) tsearch((void *) n_tree, &node_dict,
				NodeTreeLabelCompare);
		if (ntree1->ref == NULL) {
		...
		ntree2 = *(struct node_tree **) tsearch((void *) n_tree, &node_dict,
				NodeTreeLabelCompare);
		if (ntree2->ref == NULL) {
	...
	// Done
	tdestroy((void*) node_dict, (void (*)(void *))FreeNodeTree);return
root	;
}
...
GetNodeDict(char *label, void *dict) {
	...
	treeNode = tfind((void *) tempTreeNode, &dict, NodeTreeLabelCompare);
...
// ---------------------------------------------------------------------
struct node_gra *
DequeueOne(struct node_bfs *list, struct node_bfs *one, int *size) {
	struct node_bfs *temp;
	struct node_bfs *p;
...
// ---------------------------------------------------------------------
struct node_bfs *
RenewQueue(struct node_bfs *list, struct node_bfs *lp, int *size, int d) {
 ...
 */
struct node_gra *
RandomizeSymmetricNetwork(struct node_gra *net, double times, gsl_rng *gen) {
	int i;
	int nlink = 0;
...
//	nodes = CountNodes(root);
	...
	while (p->next != NULL) {
		...
		while ((n_pred = CountPredecessors(lp)) != 0) {
			ppre = lp->pred;
			while (ppre != NULL) {
				bet_loc[((ppre->ref)->ref)->num] += bet_loc[(lp->ref)->num]
						/ (double) n_pred;
	...
	while ((s = s->next) != NULL) {
		...
		while (!stack_empty(S)) {
			...
			while (!queue_empty(P[w->num])) {
				...
				delta[v->num] += (1. + delta[w->num]) * sigma[v->num]
						/ sigma[w->num];
			}
			if (w->num != s->num)
				w->dvar1 += delta[w->num];
...
// ???????????
// ---------------------------------------------------------------------
int AreConnectedList(struct node_gra *root, struct node_gra *n1,
		int cluslis[]) {
		...
		do {
			...
			while (lp2->next != NULL) {
				if (AreConnectedList(root_cop,
						GetNode(((lp2->next)->ref)->num, root_cop), this_net)
						== 0) {
 ...
 */
struct node_gra *
GetLargestStronglyConnectedSet(struct node_gra *root, int thres) {
	struct node_gra *root_cop = NULL;
		...
		do {
			...
			while (lp2->next != NULL) {
				if (AreConnectedList(root_cop,
					...
					CopyAdjacencyList(GetNode(((lp2->next)->ref)->num, root),
							temp);
					this_net[temp->num] = 1;
					selected[temp->num] = 1;
					anod++;
...
GetLargestWeaklyConnectedSet(struct node_gra *root, int thres) {
	...
	do {
		...
		while (p3->next != NULL) {
			...
			if ((AreConnectedList(root_cop, GetNode(p3->num, root_cop),
					this_net) == 1) && (selected[p3->num] == 0)) {
				...
				do {
					...
					lp2 = lp;
					while (lp2->next != NULL) {
						temp = CreateNodeGraph(root_loc,
								((lp2->next)->ref)->label);
						temp->num = lp2->next->ref->num;
						CopyAdjacencyList(
								GetNode(((lp2->next)->ref)->num, root), temp);
						this_net[temp->num] = 1;
						selected[temp->num] = 1;
						anod++;
						lp2 = lp2->next;
...
int GetAllConnectedSets(struct node_gra *network, struct node_gra **net_list) {
	...
	do {
		...
		do {
			...
			lp2 = lp;
			while (lp2->next != NULL) {
				if (AreConnectedList(network_cop,
						GetNode(((lp2->next)->ref)->num, network_cop), thisNet)
						== 0) {
					DequeueOne(list, lp2, size);
				} else {
					temp = CreateNodeGraph(network_loc,
							((lp2->next)->ref)->label);
...
void CompareTwoNetworks(struct node_gra *netA, struct node_gra *netB,
	...
	*nB_lin = 0;
	...
	while ((pA = pA->next) != NULL) {
		...
		ntreeA = *(struct node_tree **) tsearch((void *) n_tree, &node_dictA,
				NodeTreeLabelCompare);
		ntreeA->ref = pA;
		(*nA_nod) += 1;
	}
	...
	while ((pB = pB->next) != NULL) {
		...
		ntreeB = *(struct node_tree **) tsearch((void *) n_tree, &node_dictB,
				NodeTreeLabelCompare);
		ntreeB->ref = pB;
		(*nB_nod) += 1;
	...
	*p_nBA = (double) (*ncom_nod) / (double) (*nA_nod);
	...
	while ((pA = pA->next) != NULL) {
		...
		while ((lA = lA->next) != NULL) {
			...
			if (((n1 = GetNodeDict(pA->label, node_dictB)) != NULL) && ((n2 =
					GetNodeDict(lA->ref->label, node_dictB)) != NULL)) {
				ntot_lin++;
				(*nA_lin) += 1;
	...
	while ((pB = pB->next) != NULL) {
		...
		while ((lB = lB->next) != NULL) {
			/* If both nodes exist in both networks... */
			if (((n1 = GetNodeDict(pB->label, node_dictA)) != NULL) && ((n2 =
					GetNodeDict(lB->ref->label, node_dictA)) != NULL)) {
				(*nB_lin) += 1;
				if (IsThereLink(n1, n2) != 1) {
					ntot_lin++;
					fprintf(stderr, "<< %s %s\n", n1->label, n2->label);
	...
	while ((p = p->next) != NULL) {
		...
		while ((n = n->next) != NULL) {
			AddAdjacency(GetNodeDict(p->label, nodeDictSum),
					GetNodeDict(n->ref->label, nodeDictSum), 1, 0, 1, 0);
		}
	}
