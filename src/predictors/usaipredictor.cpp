...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void USAIPredictor<
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t USAIPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
			...
			if (i < k) {
				...
				if (!net->isEdge(e) && mqs[ind].count(e) == 0) {
					auto sc = net->getNbCommonNeighbors(i, k)
							/ (std::sqrt(
									(double) net->getDeg(i) * net->getDeg(k))
									+ std::numeric_limits<double>::epsilon());
					mqs[ind].push(e, sc);
				}
			}
...
#define USAIPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef USAIPREDICTOR_CPP
