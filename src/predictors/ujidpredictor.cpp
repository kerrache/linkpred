...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void UJIDPredictor<
...
#pragma omp parallel for if (parallel)
	...
	for (auto it = begin; it < end; ++it) {
		...
		*(scores + (it - begin)) = nbCN
				/ (degI + degJ - nbCN + std::numeric_limits<double>::epsilon());
	}
}

template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t UJIDPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
		...
		for (auto nnit = net->neighborsBegin(j); nnit != net->neighborsEnd(j);
			...
			if (i < k) {
				...
				if (!net->isEdge(e) && mqs[ind].count(e) == 0) {
					double degI = net->getDeg(i);
					double degJ = net->getDeg(k);
					double nbCN = net->getNbCommonNeighbors(i, k);
					auto sc = nbCN
							/ (degI + degJ - nbCN
...
#define UJIDPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef UJIDPREDICTOR_CPP
