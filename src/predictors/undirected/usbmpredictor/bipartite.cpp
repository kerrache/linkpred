...
#include "linkpred/predictors/undirected/usbmpredictor/bipartite.h"
 ...
 */
struct binet *
FBuildNetworkBipart(FILE *inFile, int weight_sw, int add_weight_sw) {
	...
	while (!feof(inFile)) {
		...
		if (weight_sw == 0) {
			...
			if (fscanf(inFile, "%s %s %lf\n", label1, label2, &weight) == EOF) {
				throw std::runtime_error("Could not read from file");
			}
		}
		...
		ntree1 = *(struct node_tree **) tsearch((void *) nodeTree, &dict1,
		...
		ntree2 = *(struct node_tree **) tsearch((void *) nodeTree, &dict2,
				NodeTreeLabelCompare);
		if (ntree2->ref == NULL)
	...
	/* Free memory */
	tdestroy((void*) dict1, (void (*)(void *))FreeNodeTree);tdestroy
	((void*) dict2, (void (*)(void *))FreeNodeTree);
 ...
 */
struct binet *
BuildModularBipartiteNetwork(int *modSizes, int nodpermod, int nmod,
		double *col_prob, int S2, int mmin, int mmax, double geom_p, double p,
		gsl_rng *gen) {
	int i, j;
	struct binet *net = NULL;
...
//	int orig, dest;
	...
	for (i = 0; i < S2; i++) { /* loop over teams */
		...
		for (j = 0; j < m; j++) { /* loop over spots in a team */
			...
			if (gsl_rng_uniform(gen) < p && n_team_col < modSizes[teamcol]) { /* select node with the
			 module's color */
				n_team_col++;
				do {
					target = mod_starting[teamcol]
							+ floor(gsl_rng_uniform(gen) * modSizes[teamcol]);
...
/*
 ...
 ---------------------------------------------------------------------
 */
double SumProductsOfCommonWeightsBipart(struct node_gra *n1,
		struct node_gra *n2) {
...
CopyBipart(struct binet *binet) {
	...
	while ((p1 = p1->next) != NULL) {
		...
		ntree1 = *(struct node_tree **) tsearch((void *) ntree1, &dict1,
				NodeTreeLabelCompare);
	...
	while ((p2 = p2->next) != NULL) {
		...
		ntree2 = *(struct node_tree **) tsearch((void *) ntree2, &dict2,
				NodeTreeLabelCompare);
	...
	while ((p1 = p1->next) != NULL) {
		...
		while ((l = l->next) != NULL) {
			AddAdjacency(GetNodeDict(p1->label, dict1),
					GetNodeDict(l->nodeLabel, dict2), 0, 0, l->weight, 0);
			AddAdjacency(GetNodeDict(l->nodeLabel, dict2),
					GetNodeDict(p1->label, dict1), 0, 0, l->weight, 0);
		}
	}
...
ProjectBipart(struct binet *binet) {
	...
	while ((p1 = p1->next) != NULL) {
		...
		ntree = *(struct node_tree **) tsearch((void *) ntree, &dict,
				NodeTreeLabelCompare);
		...
		while ((p2 = p2->next) != NULL) {
			...
			if (weight > 0) {
				AddAdjacency(GetNodeDict(p1->label, dict),
						GetNodeDict(p2->label, dict), 0, 0, (double) weight, 0);
				AddAdjacency(GetNodeDict(p2->label, dict),
						GetNodeDict(p1->label, dict), 0, 0, (double) weight, 0);
			}
		}
	}

	/* Free memory and return */
	tdestroy((void*) dict, (void (*)(void *))FreeNodeTree);return
projnet	;
}
...
RandomizeBipart(struct binet *binet, double times, gsl_rng *gen) {
	...
	for (i = 0; i < niter; i++) {
		...
		} while (IsThereLink(n1, n4) == 1 || IsThereLink(n2, n3) == 1);
...
/*
 ...
 ---------------------------------------------------------------------
 */
void FPrintPajekFileBipart(char *fname, struct binet *binet, int coor_sw,
		int weight_sw) {
	struct node_gra *p = NULL;
	...
	while ((p = p->next) != NULL) {
		if (coor_sw == 1) {
			fprintf(outF, "%d \"%s\"           %lf %lf %lf\n", p->num + 1,
					p->label, p->coorX, p->coorY, p->coorZ);
		} else {
			fprintf(outF, "%d \"%s\"\n", p->num + 1, p->label);
		}
	}
...
void SAGroupSplitBipart(struct group *target_g, struct group *empty_g,
		double Ti, double Tf, double Ts, double cluster_prob, double **cmat,
		double msfac, gsl_rng *gen) {
	...
	if (cluster_sw == 1 && NGroups(split) > 1) {
		...
		while ((g = g->next) != NULL) {
			if (gsl_rng_uniform(gen) < 0.500000) { // With prob=0.5 move to empty
				p = g->nodeList;
				while ((p = p->next) != NULL) {
					MoveNode(GetNodeDict(p->nodeLabel, dict), target_g,
							empty_g);
				}
			}
		}
	...
	if (cluster_sw == 1) {
		...
		RemoveGraph(net);
		RemoveBipart(binet);
		tdestroy((void*) dict, (void (*)(void *))FreeNodeTree);} else {
			free(nlist);
		}
...
void SAGroupSplitBipartWeighted(struct group *target_g, struct group *empty_g,
		double Ti, double Tf, double Ts, double cluster_prob, double **swwmat,
		double Wafac, gsl_rng *gen) {
	struct group *glist[2], *g = NULL, *split = NULL;
	struct node_gra **nlist;
	struct node_lis *p = NULL;
	int nnod = 0;
 ...
 */
struct group *
SACommunityIdentBipart(struct binet *binet, double Ti, double Tf, double Ts,
		double fac, int ngroup, char initial_sw, int collective_sw,
	...
	double energy, energyant = 0, dE; // e;
	double T;
	int target, oldg, newg;
	double **cmat;
	int g1, g2, empty;
	...
	while ((T >= Tf) && (count < limit)) {
		...
		for (i = 0; i < cicle1; i++) {
			...
			while ((nod = nod->next) != NULL) {
				...
				dE -=
						2.
								* (cmat[nlist[target]->num][nod->node]
										- t1 * t2 * msfac);
			}
			...
			dE += 2.
					* (cmat[nlist[target]->num][nlist[target]->num]
							- t1 * t1 * msfac);
		...
		if (collective_sw == 1) {
			for (i = 0; i < cicle2; i++) {
				...
				if (empty >= 0) { /* if there are no empty groups, do nothing */
					...
					/* Split the group */
					SAGroupSplitBipart(glist[target], glist[empty], Ti, T, 0.95,
							cluster_prob, cmat, msfac, gen);
 ...
 */
struct group *
SACommunityIdentBipartWeighted(struct binet *binet, double Ti, double Tf,
	...
	while ((T >= Tf) && (count < limit)) {
		...
		case 'v':
			fprintf(stderr, "%g %lf %lf %g\n", 1.0 / T, energy,
					ModularityBipartWeighted(binet, part), T);
			break;
		case 'd':
			FPrintPartition(stderr, part, 0);
			fprintf(stderr, "%g %lf %lf %g\n", 1.0 / T, energy,
		...
		for (i = 0; i < cicle1; i++) {
			...
			while ((nod = nod->next) != NULL) {
				...
				dE -= 2.
						* (swwmat[nlist[target]->num][nod->node]
								- s1 * s2 * Wafac);
			}
			...
			dE += 2.
					* (swwmat[nlist[target]->num][nlist[target]->num]
							- s1 * s1 * Wafac);
			...
			case 'd':
				if (dE < 0 && accepted == "ACCEPTED" && (T < Ti / 1.0e6)) {
					fprintf(stderr,
							"Cicle 1 move %s: %i move %i -> %i dE=%g prob=%g T=%g\n",
							accepted.c_str(), target, oldg, newg, dE,
							exp(dE / T), T);
				}
				accepted = "rejected";
			}
		...
		if (collective_sw == 1) {
			for (i = 0; i < cicle2; i++) {
				...
				if (glist[g1]->size < nnod) {
					...
					switch (output_sw) {
					case 'd':
						if (dE < 0 && accepted == "ACCEPTED"
								&& (T < Ti / 1.0e6)) {
							fprintf(stderr,
									"Cicle 2 Merge %s: %i (sz %i) with %i (sz %i) dE=%g prob=%g T=%g\n",
									accepted.c_str(), g1, glist[g1]->size, g2,
									glist[g2]->size, dE, exp(dE / T), T);
						}
						accepted = "rejected";
					}
				} /* End of merge move */
				...
				if (empty >= 0) { /* if there are no empty groups, do nothing */
					...
					/* Split the group */
					SAGroupSplitBipartWeighted(glist[target], glist[empty], Ti,
							T, 0.95, cluster_prob, swwmat, Wafac, gen);
					...
					case 'd':
						if (-dE < 0 && accepted == "ACCEPTED"
								&& (T < Ti / 1.0e6)) {
							fprintf(stderr,
									"Cicle 2 Split %s: %i -> %i (sz %i) and %i (sz %i) dE=%g dE>-en*Epsilon/10=%i prob=%g T=%g\n",
									accepted.c_str(), target, target,
									glist[target]->size, empty,
									glist[empty]->size, -dE,
									-dE > -fabs(energyant) * EPSILON_MOD_B / 10,
									exp(-dE / T), T);
						}
						accepted = "rejected";
					}
...
double ParticipationCoefficientBipart(struct node_gra *node) {
	...
	for (i = 0; i <= maxNum; i++)
		P += (double) inGroupCount[i] * (double) inGroupCount[i]
...
void StatisticsParticipationCoefficientBipart(struct node_gra *net,
		double *PMean, double *PStddev, double *PMin, double *PMax) {
