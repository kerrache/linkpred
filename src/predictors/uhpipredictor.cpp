...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> void UHPIPredictor<
...
template<typename NetworkT, typename EdgesRandomIteratorT,
		typename ScoresRandomIteratorT, typename EdgesRandomOutputIteratorT> std::size_t UHPIPredictor<
		NetworkT, EdgesRandomIteratorT, ScoresRandomIteratorT,
			...
			if (i < k) {
				...
				if (!net->isEdge(e) && mqs[ind].count(e) == 0) {
					auto sc = net->getNbCommonNeighbors(i, k)
							/ (std::min(net->getDeg(i), net->getDeg(k))
									+ std::numeric_limits<double>::epsilon());
					mqs[ind].push(e, sc);
				}
			}
...
#define UHPIPREDICTOR_CPP
#include "linkpred/instantiations.hpp"
#undef UHPIPREDICTOR_CPP
