...
/*
 ...
 #include "linkpred/chebyshev.h"
 #include "linkpred/cheb_eval.c"
 */
 ...
 * B_{2j}/(2j)!
 */
static double hzeta_c[15] = { 1.00000000000000000000000000000,
		0.083333333333333333333333333333, -0.00138888888888888888888888888889,
...
static int gsl_sf_hzeta_e(const double s, const double q,
		gsl_sf_result * result) {
	/* CHECK_POINTER(result) */

	if (s <= 1.0 || q <= 0.0) {
		PLFIT_ERROR("s must be larger than 1.0 and q must be larger than zero",
				PLFIT_EINVAL);
	} else {
		...
		} else if ((s > max_bits && q < 1.0)
				|| (s > 0.5 * max_bits && q < 0.25)) {
			result->val = pow(q, -s);
