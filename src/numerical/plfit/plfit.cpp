...
#include "linkpred/numerical/plfit/lbfgs.h"
...
static double* extract_smaller(double* begin, double* end, double xmin,
	...
	result = (double *) calloc(counter, sizeof(double));
	if (result == 0)
		return 0;
 ...
 * memory
 */
static double** unique_element_pointers(double* begin, double* end,
		size_t* result_length) {
	double* ptr = begin;
	double** result;
	double prev_x;
	...
	result = (double **) calloc(num_elts + 1, sizeof(double*));
	if (result == 0)
		return 0;
...
static void plfit_i_perform_finite_size_correction(plfit_result_t* result,
		size_t n) {
...
static double plfit_i_logsum_continuous(double* begin, double* end,
		double xmin) {
	double logsum = 0.0;
	for (; begin != end; begin++)
		logsum += log(*begin / xmin);
...
static int plfit_i_estimate_alpha_continuous(double* xs, size_t n, double xmin,
		double* alpha) {
	double result;
	size_t m;
...
static int plfit_i_ks_test_continuous(double* xs, double* xs_end,
	...
	while (xs < xs_end) {
		double d = std::abs(1 - pow(xmin / *xs, alpha - 1) - m / n);
...
static int plfit_i_calculate_p_value_continuous(double* xs, size_t n,
	...
	num_trials = (long int) (0.25 / options->p_value_precision
			/ options->p_value_precision);
	if (num_trials <= 0) {
...
//#pragma omp for reduction(+:successes)
			...
			for (i = 0; i < num_trials; i++) {
				plfit_i_resample_continuous(xs_head, num_smaller, n,
						result->alpha, result->xmin, n, p_rng, ys);
				if (xmin_fixed) {
					plfit_estimate_alpha_continuous(ys, n, result->xmin,
							&options_no_p_value, &result_synthetic);
				} else {
					plfit_continuous(ys, n, &options_no_p_value,
							&result_synthetic);
				}
...
int plfit_estimate_alpha_continuous_sorted(double* xs, size_t n, double xmin,
	...
	PLFIT_CHECK(
			plfit_i_estimate_alpha_continuous_sorted(begin, end - begin, xmin,
					&result->alpha));
	PLFIT_CHECK(
			plfit_i_ks_test_continuous(begin, end, result->alpha, xmin,
					&result->D));
	...
	PLFIT_CHECK(
			plfit_log_likelihood_continuous(begin, end - begin, result->alpha,
					result->xmin, &result->L));
	PLFIT_CHECK(
			plfit_i_calculate_p_value_continuous(xs, n, options, 1, result));
...
int plfit_estimate_alpha_continuous(double* xs, size_t n, double xmin,
	...
	PLFIT_CHECK(
			plfit_estimate_alpha_continuous_sorted(xs_copy, n, xmin, options,
					result));
	free(xs_copy);
...
static double plfit_i_continuous_xmin_opt_evaluate(void* instance, double x) {
	plfit_continuous_xmin_opt_data_t* data =
			(plfit_continuous_xmin_opt_data_t*) instance;
...
static int plfit_i_continuous_xmin_opt_progress(void* instance, double x,
		double fx, double min, double fmin, double left, double right, int k) {
#ifdef PLFIT_DEBUG
...
static int plfit_i_continuous_xmin_opt_linear_scan(
	...
	global_best_result.L = 0;
	global_best_result.alpha = 0;
	global_best_result.p = 0;
...
int plfit_continuous(double* xs, size_t n,
	...
	uniques = unique_element_pointers(opt_data.begin, opt_data.end,
			&num_uniques);
	if (uniques == 0)
	...
	case PLFIT_GSS_OR_LINEAR:
		...
		if (num_uniques > 5) {
			...
			success = (gss(0, opt_data.num_probes - 5, &x, 0,
					...
					plfit_i_continuous_xmin_opt_progress, &opt_data, &gss_param)
					== 0);
			if (success) {
	...
	case PLFIT_STRATIFIED_SAMPLING:
		if (num_uniques >= 50) {
			...
			double **strata = (double **) calloc(num_strata, sizeof(double*));
			...
			plfit_i_continuous_xmin_opt_linear_scan(&opt_data, &best_result,
					&best_n);
	...
	*result = best_result;
	...
	PLFIT_CHECK(
			plfit_i_calculate_p_value_continuous(opt_data.begin, n, options, 0,
					result));
...
static void plfit_i_logsum_less_than_discrete(double* begin, double* end,
		double xmin, double* logsum, size_t* m) {
	double result = 0.0;
	size_t count = 0;
...
static lbfgsfloatval_t plfit_i_estimate_alpha_discrete_lbfgs_evaluate(
	...
	if (x[0] + dx <= 1.0)
		...
		g[0] = data->logsum
				+ data->m
						* (log(gsl_sf_hzeta(x[0] + dx, data->xmin))
								- log(gsl_sf_hzeta(x[0], data->xmin))) / dx;

	result = x[0] * data->logsum
			+ data->m * log(gsl_sf_hzeta(x[0], data->xmin));
...
static int plfit_i_estimate_alpha_discrete_lbfgs(double* xs, size_t n,
		double xmin, double* alpha, const plfit_discrete_options_t* options,
		plfit_bool_t sorted) {
	...
	if (sorted) {
		...
		plfit_i_logsum_less_than_discrete(xs, xs + n, xmin, &data.logsum,
				&data.m);
	}
	...
	ret = lbfgs(1, variables, /* ptr_fx = */0,
			...
			plfit_i_estimate_alpha_discrete_lbfgs_progress, &data, &param);

	if (ret < 0 && ret != LBFGSERR_ROUNDING_ERROR
			&& ret != LBFGSERR_MAXIMUMLINESEARCH && ret != LBFGSERR_MINIMUMSTEP
			&& ret != LBFGSERR_CANCELED) {
		char buf[4096];
		snprintf(buf, 4096,
				"L-BFGS optimization signaled an error (error code = %d)", ret);
...
static int plfit_i_estimate_alpha_discrete_fast(double* xs, size_t n,
		double xmin, double* alpha, const plfit_discrete_options_t* options,
		plfit_bool_t sorted) {
	...
	if (sorted) {
		return plfit_i_estimate_alpha_continuous_sorted(xs, n, xmin - 0.5,
				alpha);
	} else {
...
static int plfit_i_estimate_alpha_discrete(double* xs, size_t n, double xmin,
	...
	case PLFIT_LBFGS:
		PLFIT_CHECK(
				plfit_i_estimate_alpha_discrete_lbfgs(xs, n, xmin, alpha,
						options, sorted));
		break;

	case PLFIT_LINEAR_SCAN:
		PLFIT_CHECK(
				plfit_i_estimate_alpha_discrete_linear_scan(xs, n, xmin, alpha,
						options, sorted));
		break;

	case PLFIT_PRETEND_CONTINUOUS:
		PLFIT_CHECK(
				plfit_i_estimate_alpha_discrete_fast(xs, n, xmin, alpha,
						options, sorted));
		break;
...
static int plfit_i_ks_test_discrete(double* xs, double* xs_end,
		const double alpha, const double xmin, double* D) {
	/* Assumption: xs is sorted and cut off at xmin so the first element is
	 * always larger than or equal to xmin. */
...
#endif
			for (i = 0; i < num_trials; i++) {
				plfit_i_resample_discrete(xs_head, num_smaller, n,
						result->alpha, result->xmin, n, p_rng, ys);
				if (xmin_fixed) {
					plfit_estimate_alpha_discrete(ys, n, result->xmin,
							&options_no_p_value, &result_synthetic);
				} else {
					plfit_discrete(ys, n, &options_no_p_value,
							&result_synthetic);
				}
				if (result_synthetic.D > result->D)
					successes++;
...
int plfit_estimate_alpha_discrete(double* xs, size_t n, double xmin,
	...
	if (options->alpha_method == PLFIT_LINEAR_SCAN) {
		...
		if (options->alpha.max < options->alpha.min) {
			PLFIT_ERROR("alpha.max must be greater than alpha.min",
					PLFIT_EINVAL);
		}
		if (options->alpha.step <= 0) {
			PLFIT_ERROR("alpha.step must be positive", PLFIT_EINVAL);
	...
	PLFIT_CHECK(
			plfit_i_estimate_alpha_discrete(begin, end - begin, xmin,
					&result->alpha, options, /* sorted = */1));
	PLFIT_CHECK(
			plfit_i_ks_test_discrete(begin, end, result->alpha, xmin,
					&result->D));
	...
	PLFIT_CHECK(
			plfit_log_likelihood_discrete(begin, end - begin, result->alpha,
					result->xmin, &result->L));
	...
	*result = best_result;
	...
	PLFIT_CHECK(
			plfit_log_likelihood_discrete(xs_copy + (n - best_n), best_n,
					result->alpha, result->xmin, &result->L));
	PLFIT_CHECK(
			plfit_i_calculate_p_value_discrete(xs_copy, n, options, 0, result));
...
static int plfit_i_resample_continuous(double* xs_head, size_t num_smaller,
	...
	num_orig_samples = (size_t) plfit_rbinom(num_samples,
			num_smaller / (double) n, rng);
...
static int plfit_i_resample_discrete(double* xs_head, size_t num_smaller,
	...
	/* Draw the remaining samples from the fitted distribution */
	PLFIT_CHECK(
			plfit_rzeta_array((long int ) xmin, alpha,
...
int plfit_calculate_p_value_continuous(double* xs, size_t n,
	...
	PLFIT_CHECK(plfit_i_copy_and_sort(xs, n, &xs_copy));
	PLFIT_CHECK(
			plfit_i_calculate_p_value_continuous(xs_copy, n, options,
					xmin_fixed, result));
	free(xs_copy);
...
int plfit_calculate_p_value_discrete(double* xs, size_t n,
	...
	PLFIT_CHECK(plfit_i_copy_and_sort(xs, n, &xs_copy));
	PLFIT_CHECK(
			plfit_i_calculate_p_value_discrete(xs_copy, n, options, xmin_fixed,
					result));
	free(xs_copy);
