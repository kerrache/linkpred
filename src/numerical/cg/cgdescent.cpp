...
int CGDescent::cg_descent(double *x, INT n, cg_stats *Stat, cg_parameter *UParm,
		double grad_tol, double *Work, bool init) {
	INT i = 0, iter = 0, IterRestart = 0, maxit = 0, nrestart = 0, nrestartsub =
			0;
	int nslow = 0, slowlimit = 0, IterQuad = 0, status = 0, PrintLevel = 0,
			QuadF = 0;
	double delta2, Qk, Ck, fbest, gbest, f, ftemp, gnorm, xnorm, gnorm2, dnorm2,
			denom, t, dphi, dphi0, alpha, ykyk, ykgk, dkyk, beta = 0, QuadTrust,
			tol, *d = nullptr, *g = nullptr, *xtemp = nullptr, *gtemp = nullptr,
			*work = nullptr;

	/* new variables added in Version 6.0 */
	int l1 = 0, l2 = 0, j = 0, k = 0, mem = 0, memsq = 0, memk = 0, memk_begin =
			0, mlast = 0, mlast_sub = 0, mp = 0, mp_begin = 0, mpp = 0,
			nsub = 0, spp = 0, spp1 = 0, SkFstart = 0, SkFlast = 0,
			Subspace = 0, UseMemory = 0, Restart = 0, LBFGS = 0,
			InvariantSpace = 0, IterSub = 0, NumSub = 0, IterSubRestart = 0,
			FirstFull = 0, SubSkip = 0, SubCheck = 0, StartSkip = 0,
			StartCheck = 0, DenseCol1 = 0, NegDiag = 0, memk_is_mem = 0, d0isg =
					0, qrestart = 0;
	double gHg, scale, gsubnorm2 = 0, ratio, stgkeep = 0, alphaold, zeta, yty,
			ytg, t1, t2, t3, t4, *Rk = nullptr, *Re = nullptr, *Sk = nullptr,
			*SkF = nullptr, *stemp = nullptr, *Yk = nullptr, *SkYk = nullptr,
			*dsub = nullptr, *gsub = nullptr, *gsubtemp = nullptr, *gkeep =
					nullptr, *tau = nullptr, *vsub = nullptr, *wsub = nullptr;
...
//	n5 = n % 5;
	...
	Com.f0 = f + f;
	Com.SmallCost = std::abs(f) * Parm->SmallCost;
	xnorm = cg_inf(x, n);
	...
	for (iter = 1; iter <= maxit; iter++) {
		...
		if (Parm->QuadStep) {
			...
			if (((t > Parm->QuadCutOff) && (std::abs(f) >= Com.SmallCost))
					|| QuadF) {
				if (QuadF) {
		...
		Ck = Ck + (std::abs(f) - Ck) / Qk; /* average cost magnitude */

		if (Com.PertRule)
			Com.fpert = f + Com.eps * std::abs(f);
		else
			Com.fpert = f + Com.eps;
		...
		if (-alpha * dphi0 <= Parm->feps * std::abs(f)) {
			status = 1;
			goto Exit;
		}
		...
		t = alpha * (dphi + dphi0);
		if (std::abs(t) <= Parm->qeps * MIN(Ck, ONE))
			QuadTrust = ZERO;
		else
			QuadTrust = std::abs((2.0 * (f - Com.f0) / t) - ONE);
		...
		IterRestart++;
		if (!Com.AWolfe) {
			if (std::abs(f - Com.f0) < Parm->AWolfeFac * Ck) {
				Com.AWolfe = true;
				if (Com.Wolfe)
					Restart = true;
			}
			...
			} else /* in full space */
			{
				...
				if (memk < mem) {
					...
					if (memk == 0) {
						...
						if ((std::abs(alpha - 5.05) > 4.95)
								|| (std::abs(alphaold - 5.05) > 4.95)) {
						...
						cg_Yk(Yk + spp, gsub, wsub, nullptr, memk);
				...
				} else /* memk = mem */
				{
					...
					cg_Yk(Yk + spp, gsub, wsub, nullptr, mem + 1);
...
//					IterSubStart = IterSub;
		...
		if (LBFGS) {
			...
			if (IterRestart == nrestart) /* restart the l-bfgs method */
			{
				...
				gnorm2 = cg_update_2(g, gtemp, nullptr, n);
				...
				dnorm2 = cg_update_2(nullptr, gtemp, d, n);
		...
		else if (Subspace) /* compute search direction in subspace */
		{
			...
			gnorm = cg_update_inf(g, gtemp, nullptr, n);
			...
			} else /* continue in subspace without restart */
			{
				...
				if (IterSubRestart > 0) /* not first iteration in subspace  */
				{
					...
					yty = cg_dot0(Yk + mlast_sub * mem, Yk + mlast_sub * mem,
							nsub);
					if (yty > ZERO) {
						scale = SkYk[mlast_sub] / yty;
					}
		...
		else /* compute the search direction in the full space */
		{
			...
			} else if (!FirstFull) /* normal fullspace step*/
			{
				...
				if (UseMemory) {
					...
					dnorm2 = cg_update_d(d, g, beta, nullptr, n);
				} else {
					/* update search direction d = -g + beta*dold, and
...
/* =========================================================================
 ...
 ========================================================================= */
int CGDescent::cg_Wolfe(double alpha, /* stepsize */
...
/* =========================================================================
 ...
 ========================================================================= */
int CGDescent::cg_tol(double gnorm, /* gradient sup-norm */
cg_com *Com /* cg com */
) {
	...
	if (Com->Parm->StopRule) {
		if (gnorm <= Com->tol)
			return (1);
	} else if (gnorm <= Com->tol * (ONE + std::abs(Com->f)))
		return (1);
	return (0);
}
...
/* =========================================================================
 ...
 ========================================================================= */
int CGDescent::cg_line(cg_com *Com /* cg com structure */
) {
	...
	/*Find initial interval [a,b] such that
	 da <= 0, db >= 0, fa <= fpert = [(f0 + eps*std::abs (f0)) or (f0 + eps)] */
	rho = Com->rho;
	ngrow = 1;
	while (db < ZERO) {
		if (!qb) {
...
/* =========================================================================
 ...
 ========================================================================= */
int CGDescent::cg_contract(double *A, /* left side of bracketing interval */
...
cg_com *Com /* cg com structure */
	...
	double a, alpha, b, old, da, db, df, dold, f, fa, fb, f1, fold, t, width;
	const char *s;
	cg_parameter *Parm;
...
//	d1 = db;
	...
	if (Com->PertRule) {
		if (t != ZERO) {
			Com->eps = Parm->egrow * (f1 - t) / std::abs(t);
			Com->fpert = t + std::abs(t) * Com->eps;
		} else
			Com->fpert = 2. * f1;
	} else {
...
int CGDescent::cg_evaluate(const char *what, /* fg = evaluate func and grad, g = grad only,f = func only*/
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_cubic(double a, double fa, /* function value at a */
double da, /* derivative at a */
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_matvec(double *y, /* product vector */
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_trisolve(double *x, /* right side on input, solution on output */
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_inf(double *x, /* vector */
INT n /* length of vector */
	...
	if (n < IDAMAX_START) {
		...
		return (std::abs(x[i - 1])); /* adjust for fortran indexing */
	}
#endif
}
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_scale0(double *y, /* output vector */
double *x, /* input vector */
double s, /* scalar */
int n /* length of vector */
) {
...
/* =========================================================================
 ...
 compute y = s*x where s is a scalar
 ========================================================================= */
void CGDescent::cg_scale(double *y, /* output vector */
double *x, /* input vector */
double s, /* scalar */
INT n /* length of vector */
) {
...
/* =========================================================================
 ...
 Compute x = x + alpha d
 ========================================================================= */
void CGDescent::cg_daxpy0(double *x, /* input and output vector */
double *d, /* direction */
...
/* =========================================================================
 ...
 Compute x = x + alpha d
 ========================================================================= */
void CGDescent::cg_daxpy(double *x, /* input and output vector */
double *d, /* direction */
double alpha, /* stepsize */
INT n /* length of the vectors */
) {
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_dot0(double *x, /* first vector */
...
/* =========================================================================
 ...
 Compute dot product of x and y, vectors of length n
 ========================================================================= */
double CGDescent::cg_dot(double *x, /* first vector */
double *y, /* second vector */
INT n /* length of vectors */
) {
#ifdef NOBLAS
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_copy0(double *y, /* output of copy */
...
/* =========================================================================
 ...
 Copy vector x into vector y
 ========================================================================= */
void CGDescent::cg_copy(double *y, /* output of copy */
double *x, /* input of copy */
INT n /* length of vectors */
) {
#ifdef NOBLAS
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_step(double *xtemp, /*output vector */
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_init(double *x, /* input and output vector */
double s, /* scalar */
INT n /* length of vector */
) {
	INT i, n5;
...
/* =========================================================================
 ...
 set d = -gnew
 ========================================================================= */
double CGDescent::cg_update_2(double *gold, /* old g */
double *gnew, /* new g */
double *d, /* d */
INT n /* length of vectors */
) {
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_update_inf(double *gold, /* old g */
double *gnew, /* new g */
double *d, /* d */
INT n /* length of vectors */
) {
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_update_ykyk(double *gold, /* old g */
double *gnew, /* new g */
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_update_inf2(double *gold, /* old g */
double *gnew, /* new g */
double *d, /* d */
...
/* =========================================================================
 ...
 ========================================================================= */
double CGDescent::cg_update_d(double *d, double *g, double beta, double *gnorm2, /* 2-norm of g */
INT n /* length of vectors */
) {
	INT i, n5;
	double dnorm2, s, t;
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_Yk(double *y, /*output vector */
...
/* =========================================================================
 ...
 of cg_descent is nullptr, this routine is called by cg_descent automatically.
 ...
 =========================================================================*/
void CGDescent::cg_default(cg_parameter *Parm) {
...
/* =========================================================================
 ...
 ========================================================================= */
void CGDescent::cg_printParms(cg_parameter *Parm) {
	printf("PARAMETERS:\n");
	printf("\n");
...
/*
 ...
 In the simplest case, the user can use nullptr for the
