...
int main(int argc, char*argv[]) {
	...
	for (std::size_t i = 0; i < nbTests; i++) {
		for (double ratio = 0.1; ratio <= 0.91; ratio += 0.2) {
	...
	std::cout << "#--------------------------------------" << std::endl;
	auto smallNet = NetworkManipulator<>::createTestData(fullNet, 0.9, 0, false,
			true, 0, true, 0, FN, TN, rng.getInt()).getObsNet();
	std::cerr << smallNet->getNbEdges() << std::endl;
	std::size_t maxNbEdges = (fullNet->getNbNodes()
			* (fullNet->getNbNodes() - 1)) / 2;
	for (std::size_t i = 0; i < nbTests; i++) {
		for (double ratio = 0.1; ratio <= 0.91; ratio += 0.2) {
			double rt = (ratio
					* (fullNet->getNbEdges() - smallNet->getNbEdges()))
					/ maxNbEdges;
			//std::cerr << rt << std::endl;
			auto testData = NetworkManipulator<>::createTestData(smallNet, 0,
					rt, false, true, 0, true, 0, TP, FP, rng.getInt());
			std::cout << ratio << "\t";
			for (auto it = testData.getObsNet()->nodesDegBegin();
					it != testData.getObsNet()->nodesDegEnd(); ++it) {
				std::cout << it->second << "\t";
