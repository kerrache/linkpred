...
int main(int argc, char*argv[]) {
	...
	Dijkstra<> dijkstra(net);
	{
		...
		std::cout << "Src\tDst\tSiml\tnbHops" << std::endl;

		for (auto sit = net->nodesBegin(); sit != net->nodesEnd(); ++sit) {
			//auto nnzSMap = calc.getNnzSimlMap(sit->first);
			auto nnzSMap = calc.getNnzSimlMapNoNeighb(sit->first);
			for (auto it = nnzSMap->begin(); it != nnzSMap->end(); ++it) {
				std::cout << sit->second << "\t" << net->getLabel(it->first)
