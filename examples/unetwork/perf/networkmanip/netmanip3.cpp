...
int main(int argc, char *argv[]) {
	...
	auto testData = NetworkManipulator<UNetwork<unsigned int>>::createTestData(
			net, 0.4, 0.3, true, true, 0, true, 0, FN, TN, 777, false);
	...
	std::cout << "True positive links:" << std::endl;
	for (auto it = eg->tpBegin(); it != eg->tpEnd(); ++it) {
		std::cout << net->getLabel(net->start(*it)) << "\t"
				<< net->getLabel(net->end(*it)) << std::endl;
	}
	std::cout << "True negative links:" << std::endl;
