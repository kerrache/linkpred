...
int main(int argc, char *argv[]) {
	...
	std::cout << "Positive links in the test set (false negatives):"
			<< std::endl;
//	int cpt = 0;
//	for (auto it = testData.posStrmBegin(); it != testData.posStrmEnd(); ++it) {
//		//std::cout << net->getLabel(net->start(*it)) << "\t" << net->getLabel(net->end(*it)) << std::endl;
//		cpt++;
//	}
//	std::cout << "# pos edges: " << cpt << std::endl;
//	std::cout << "Negative links in the test set (true negatives):"	<< std::endl;
//	cpt =0;
//	for (auto it = testData.negStrmBegin(); it != testData.negStrmEnd(); ++it) {
//		//std::cout << net->getLabel(net->start(*it)) << "\t" << net->getLabel(net->end(*it)) << std::endl;
//		cpt++;
//	}
...
//	std::cout << "# pos edges: " << cpt << std::endl;
	std::cout << "Negative links in the test set (true negatives):"
			<< std::endl;
	auto eg = testData.getEg();
	int cpt = 0;
	for (auto it = eg->tnBegin(); it != eg->tnEnd(); ++it) {
		//std::cout << net->getLabel(net->start(*it)) << "\t" << net->getLabel(net->end(*it)) << std::endl;
		cpt++;
	}
	std::cout << "# neg edges: " << cpt << std::endl;
