...
struct binet *BuildModularBipartiteNetwork(int *mod_size, int nodpermod,
		int nmod, double *col_prob, int S2, int mmin, int mmax, double geom_p,
		double p, gsl_rng *gen);
struct binet *FBuildNetworkBipart(FILE *inFile, int weight_sw,
		int add_weight_sw);
...
struct binet *RandomizeBipart(struct binet *binet, double times, gsl_rng *gen);
...
struct group *SACommunityIdentBipart(struct binet *binet, double Ti, double Tf,
		double Ts, double fac, int ngroup, char initial_sw, int collective_sw,
		char output_sw, gsl_rng *gen);
struct group *SACommunityIdentBipartWeighted(struct binet *binet, double Ti,
		double Tf, double Ts, double fac, int ngroup, char initial_sw,
		int collective_sw, char output_sw, gsl_rng *gen);
...
void StatisticsParticipationCoefficientBipart(struct node_gra *net,
		double *theMean, double *theStddev, double *theMin, double *theMax);
