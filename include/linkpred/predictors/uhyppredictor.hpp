...
template<typename NetworkT = UNetwork<>,
		...
		typename EdgesRandomOutputIteratorT = typename std::vector<
				typename NetworkT::EdgeType>::iterator> class UHYPPredictor: public ULPredictor<
	...
	public:
		 ...
		 */
		std::shared_ptr<Node> findNode(NodeIdType id) {
...
protected:
	...
	std::map<NodeIdType, std::pair<double, double>> nodesCoord; /**< Nodes coordinates. */
...
public:
	 ...
	 */
	UHYPPredictor(std::shared_ptr<NetworkT const> net, long int seed) :
