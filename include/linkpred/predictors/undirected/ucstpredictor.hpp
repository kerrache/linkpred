...
template<typename Network = UNetwork<>,
		...
		typename EdgeRndOutIt = typename std::vector<typename Network::Edge>::iterator> class UCSTPredictor: public ULPredictor<
...
public:
	 ...
	 * @param seed Seed for the random number generator. This is used to select randomly k edges in the method top. Different calls to top return different set of edges, which the correct behavior.
	 */
	UCSTPredictor(std::shared_ptr<Network const> net, long int seed) :
			ULPredictor<Network, EdgeRndIt, ScoreRndIt, EdgeRndOutIt>(net), rng(
					seed) {
		name = "CST";
	}
