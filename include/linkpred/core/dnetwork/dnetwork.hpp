 ...
 * @tparam EdgeT Type of edges. This must be an unsigned integral type having at least double the size of @p NodeID \ref DNetwork.
 */
template<typename LabelT = std::string, typename NodeIDT = unsigned int,
		typename EdgeT = unsigned long long int> class DNetwork {
...
public:
	...
	class NodeDegIt: public std::iterator<std::random_access_iterator_tag,
			const std::pair<NodeID, std::pair<std::size_t, std::size_t>>,
			long int> {
		friend class DNetwork; /**< DNetwork is a friend. */

	protected:
		DNetwork const &net; /**< The network on which to iterate. */
		typename DNetwork::NodeIt nit; /**< A node iterator. */
		std::pair<NodeID, std::pair<std::size_t, std::size_t>> nodeDeg; /**< To store current node in and out degrees. */
		 ...
		 */
		NodeDegIt(DNetwork const &net, typename DNetwork::NodeIt nit) :
				net(net), nit(nit) {
			nodeDeg = std::make_pair(0, std::make_pair(0, 0));
		}

	public:
		using pointer = typename std::iterator<std::random_access_iterator_tag, const std::pair<NodeID, std::pair<std::size_t, std::size_t>>, long int>::pointer; /**< The pointer type associated with the iterator. */
		using reference = typename std::iterator<std::random_access_iterator_tag, const std::pair<NodeID, std::pair<std::size_t, std::size_t>>, long int>::reference; /**< The reference type associated with the iterator. */
		using difference_type = typename std::iterator<std::random_access_iterator_tag, const std::pair<NodeID, std::pair<std::size_t, std::size_t>>, long int>::difference_type; /**< The difference type associated with the iterator. */
	...
	protected:
		 ...
		 */
		NonEdgeIt(DNetwork const &net, std::size_t ord, std::size_t j) :
				net(net), ord(ord), j(j) {
		 ...
		 * @param ord The order of the initial edge.
		 */
		NonEdgeIt(DNetwork const &net, std::size_t ord) :
				net(net), ord(ord) {
			nbNonEdges = net.getNbNonEdges();
			jSynced = false;
		}
	...
	protected:
		typename DNetwork::NodeIt nit; /**< Node iterator. */
		typename DNetwork::NodeIt end; /**< The end of nodes. */
		 ...
		 */
		RndNodeIt(typename DNetwork::NodeIt nit, typename DNetwork::NodeIt end,
	...
	protected:
		typename DNetwork::EdgeIt eit; /**< Edge iterator. */
		typename DNetwork::EdgeIt end; /**< The end of edges. */
		 ...
		 * @param seed The random number generator's seed.
		 */
		RndEdgeIt(typename DNetwork::EdgeIt eit, typename DNetwork::EdgeIt end,
				double ratio, long int seed) :
				end(end), ratio(ratio), rng(seed) {
	...
	protected:
		 ...
		 * @param ord The order of the initial edge.
		 */
		RndNonEdgeIt(DNetwork const &net, std::size_t ord) :
				net(net), ord(ord) {
		}
		 ...
		 * @param seed The random number generator's seed.
		 */
		RndNonEdgeIt(DNetwork const &net, std::size_t ord, double ratio,
				long int seed) :
...
protected:
	...
	std::vector<Edge> oja; /**< Outgoing adjacency matrix column index. */
	std::vector<std::size_t> iia; /**< Incoming adjacency matrix row index. */
	std::vector<Edge> ija; /**< Incoming adjacency matrix column index. */
...
private:
	std::set<Edge> outEdgeSet; /**< Temporary edge list used before assembly (outgoing). */
	std::set<Edge> inEdgeSet; /**< Temporary edge list used before assembly (incoming). */
...
public:
	...
	inline std::size_t coupleAtOrd(std::size_t ord) const {
		NodeID i = ord / (nbNodes - 1);
		NodeID j = ord % (nbNodes - 1);
		if (j < i) {
			return makeEdge(i, j);
		} else {
	...
	RndEdgeIt rndEdgesBegin(double ratio, long int seed) const {
		return RndEdgeIt(oja.begin(), oja.end(), ratio, seed);
	}
	...
	RndEdgeIt rndEdgesEnd() const {
		return RndEdgeIt(oja.end(), oja.end(), 0, 0);
	}
	 ...
	 */
	static std::shared_ptr<DNetwork<Label, NodeID, Edge>> read(
